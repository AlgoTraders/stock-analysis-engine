...
history_loc=${transport_scheme}${s3_history_bucket}/${dataset_name}
...
echo ""
echo ""
echo "sa -t ${ticker} -b ${backtest_loc} -g ${algo_module_path} -p ${history_loc}"
sa -t ${ticker} -b ${backtest_loc} -g ${algo_module_path} -p ${history_loc}

echo ""
echo "aws --endpoint-url http://localhost:9000 s3 ls s3://algohistory | grep ${dataset_name}"
aws --endpoint-url http://localhost:9000 s3 ls s3://algohistory | grep ${dataset_name}
echo ""
