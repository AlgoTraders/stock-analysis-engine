...
namespace="ae"
values="./ae-intraday/values.yaml"
recreate="0"
...
if [[ "${1}" != "" ]]; then
    if [[ "${1}" == "-r" ]]; then
        ...
        if [[ ! -e ${1} ]]; then
            err "Failed to find intraday job values file: ${1}"
            exit 1
        fi
        values=${1}
    fi
...
if [[ "${2}" != "" ]]; then
    if [[ "${2}" == "-r" ]]; then
        ...
        if [[ ! -e ${2} ]]; then
            err "Failed to find intraday job values file: ${2}"
            exit 1
        fi
        values=${2}
    fi
...
if [[ "${recreate}" == "1" ]]; then
    anmt "deleting previous ae intraday job"
    helm delete --purge ae-intraday
    pod_name="intraday"
...
anmt "installing ae intraday job"
good "helm install --name=ae-intraday ./ae-intraday --namespace=${namespace} -f ${values}"
helm install \
    --name=ae-intraday \
    ./ae-intraday \
    --namespace=${namespace} \
