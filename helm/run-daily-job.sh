...
if [[ "${1}" != "" ]]; then
    if [[ "${1}" == "-r" ]]; then
        ...
        if [[ ! -e ${1} ]]; then
            err "Failed to find daily job values file: ${1}"
            exit 1
        fi
        values=${1}
    fi
...
if [[ "${2}" != "" ]]; then
    if [[ "${2}" == "-r" ]]; then
        ...
        if [[ ! -e ${2} ]]; then
            err "Failed to find daily job values file: ${2}"
            exit 1
        fi
        values=${2}
    fi
...
if [[ "${recreate}" == "1" ]]; then
    anmt "deleting previous ae daily job"
    helm delete --purge ae-daily
    pod_name="daily"
...
anmt "installing ae daily job"
good "helm install --name=ae-daily ./ae-daily --namespace=${namespace} -f ${values}"
helm install \
    --name=ae-daily \
    ./ae-daily \
    --namespace=${namespace} \
